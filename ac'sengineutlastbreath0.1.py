#!/usr/bin/env python3
# ac's engine utlastbreath 0.1 — files = off • python 3.14 • 60 fps
...
# Last Breath Megalovania OST — procedural synth → base64 WAV in RAM (files = off)
SR = 22050
_LB_NOTE = {
...
def _lb_freq(name: str) -> float:
    octv = int(name[-1])
    key = name[:-1]
    return 440.0 * (2 ** ((_LB_NOTE[key] + (octv - 4) * 12) / 12))


def _lb_osc(wave: str, freq: float, t: float, duty: float = 0.5) -> float:
    phase = (t * freq) % 1.0
    if wave in ("sq", "pulse"):
...
def _lb_vib(freq: float, t: float, depth: float = 0.015, rate: float = 5.5) -> float:
    return freq * (1.0 + depth * math.sin(t * rate * math.tau))


def _lb_seq(text: str) -> list[tuple[str, float]]:
...
def _lb_kick(buf: list[float], pos: int, vol: float = 0.30) -> None:
...
def _lb_snare(buf: list[float], pos: int, vol: float = 0.10) -> None:
...
def _lb_hat(buf: list[float], pos: int, vol: float = 0.022) -> None:
    length = min(90, len(buf) - pos)
    for j in range(length):
        buf[pos + j] += (random.random() * 2.0 - 1.0) * vol * (1.0 - j / length)


def _build_megalovania_lb_wav() -> bytes:
    """Last Breath Megalovania-style battle loop — inspired by UI Epic / Toby Fox."""
    bpm = 120.0
    spb = 60.0 / bpm
    melody = _lb_seq(
    ...
    bass = _lb_seq("D3:1 D3:1 A2:1 A2:1 D3:1 D3:1 F3:1 G3:1 A3:1 A3:1 D3:1 D3:1 F3:1 G3:1 A3:2")
    arp = [("D4", "F4", "A4", "D5"), ("A3", "C4", "E4", "A4"), ("Bb3", "D4", "F4", "Bb4"), ("G3", "Bb3", "D4", "G4")]
    ...
    for note, beats in melody:
        ...
        if note != "R":
            ...
            for i in range(note_samples):
                ...
                buf[pos + i] += _lb_osc("pulse", _lb_vib(freq, t), t, 0.25) * 0.24 * env
        pos += note_samples
    ...
    while pos < samples:
        ...
        for i in range(note_samples):
            ...
            buf[pos + i] += _lb_osc("tri", freq, (pos + i) / SR) * 0.20 * env
        pos += note_samples
    ...
    for i in range(0, samples, sixteenth):
        ...
        for j in range(min(sixteenth, samples - i)):
            ...
            buf[i + j] += _lb_osc("thin", freq, (i + j) / SR) * 0.09 * env
        arp_t += 1
    ...
    while i < samples:
        if beat_n % 4 in (0, 2):
            _lb_kick(buf, i, 0.32)
        if beat_n % 4 in (1, 3):
            _lb_snare(buf, i, 0.11)
        for h in range(2):
            _lb_hat(buf, i + h * beat // 2)
        i += beat
        beat_n += 1
...
MEGALOVANIA_LB_B64 = base64.b64encode(_build_megalovania_lb_wav()).decode("ascii")


class LBMusic:
    """Looping Last Breath Megalovania — base64 WAV in RAM, files = off."""
    ...
    def __init__(self) -> None:
        ...
        try:
            wav = base64.b64decode(MEGALOVANIA_LB_B64)
    ...
    def play_loop(self) -> None:
        if self.ok and self.sound and not self.mute:
            self.sound.play(-1)
...
pygame.display.set_caption("ac's last breath engine my take 0.1 files = off")
